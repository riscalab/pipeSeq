...
then 
    ...
    elif [ "$genomeMap" == "Lambda_NEB" ]
    then
        genomeRef="/rugpfs/fs0/risc_lab/store/risc_data/downloaded/Lambda_cI857ind_1_Sam_7/genome/Sequence/Bowtie2Index/genome"
        if [ -z "$blacklist" ]
        then
            blacklist="None"
        fi
	...
	elif [ "$genomeMap" == "rDNA_S288c" ]
    then
        genomeRef="/rugpfs/fs0/risc_lab/store/risc_data/downloaded/rDNA_S288c/genome/Sequence/Bowtie2Index/genome"
        if [ -z "$blacklist" ]
        then
            blacklist="None"
        fi
    else
        echo "unrecognized genome.\navailable genomes: hg38, hg19, mm9, mm10, dm6, EF2, HBV, HBV_PNAS, GRCg7b, GRCg7w, hfxDS2, ecoli_mg1655, Lambda_NEB, T7_Phage, sacCer3, sacCer3w2M, rDNA_S288c."
