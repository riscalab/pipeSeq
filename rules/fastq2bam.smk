    ...
    run:
        shell("""
              ...
              if [ $f1 == $l14 ] && [ $f2 == $l24 ] then
                  {workflow.basedir}/scripts/pyadapter_trimP3V2.py -a {input.r1} -b {input.r2} > {config[sample]}/{pre_tag}_adapter_trim.log
              else
                  {workflow.basedir}/scripts/pyadapter_trimP3.py -a {input.r1} -b {input.r2} > {config[sample]}/{pre_tag}_adapter_trim.log
              fi
            """)
