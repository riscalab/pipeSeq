...
rule sortBam:
    input:
        "{config[sample]}/{config[sample]}_{post_tag}.trim.bam"
    output:
        "{config[sample]}/{config[sample]}_{post_tag}.trim.st.bam"
    params:
        "{config[sample]}_{post_tag}.trim.st.bam"
    run:
...
rule filterBam:
    input:
        "{config[sample]}/{config[sample]}_{post_tag}.trim.st.bam"
    output:
        "{config[sample]}/{config[sample]}_{post_tag}.trim.st.all.bam"
    params:
        chrs = "samtools view -H {config[sample]}/{config[sample]}_{post_tag}.trim.st.bam | grep chr | cut -f2 | sed 's/SN://g' | grep -v chrM | grep -v _gl | grep -v Y | grep -v hap | grep -v random | grep -v v1 | grep -v v2",
        chrM = "{config[sample]}/{config[sample]}_{post_tag}.trim.st.chrM.bam",
...
rule filter_removeDups_and_enrichTSS:
    input:
        "{config[sample]}/{config[sample]}_{post_tag}.trim.st.all.bam"
    output:
        "{config[sample]}/{config[sample]}_{post_tag}{ext,.*}.rmdup.bam"
    params:
    ...
    run:
        ...
        if os.path.exists(config["blacklist"]):
            ...
            ftp = "{config[sample]}/{config[sample]}_{wildcards.post_tag}.trim.st.all.blft.bam"
